...
- DNAT 없이 서버에 직접 연결하는 환경별 `1.0.68` 펌웨어 생성 및 순정 `1.0.66` 복원 지원
- `1.0.68`에서 전압·전체 전류와 채널별 누적 전력량·전류·온도 상태 보고 지원
...
공유기에서 목적지 기반 DNAT를 사용할 수 없다면 대시보드의 **Direct-local F/W Patch** 기능을 사용할 수 있습니다. 이 기능은 이미지에 포함된 순정 `1.0.66`을 서버 환경에 맞게 패치하여 `1.0.68`을 실행 시점에 생성합니다. 미리 만들어진 공통 패치 펌웨어를 배포하는 방식이 아니며, 생성된 파일에는 사용자의 Local Server IP가 들어갑니다.

### 순정 펌웨어와 로컬 펌웨어의 차이

| 항목 | 순정 `1.0.66` | Direct-local `1.0.68` |
| --- | --- | --- |
...
| 상태 데이터 | 전체·채널 전원 상태, 순간 전력, 전체 누적 전력량 | `1.0.66` 상태값 + 전체 전압·전체 전류, 채널별 누적 전력량·전류·온도 |
| Home Assistant 센서 | 전체·채널 순간 전력, 전체 누적 전력량 | `1.0.66` 센서 + 전체 전압·전체 전류, 채널별 누적 전력량·전류·온도 |
| 펌웨어 파일 | `comMTTL-W01_1.0.66.fwr` | `comMTTL-W01_1.0.68.fwr` |

MEF는 기기 등록·상태 및 OTA 확인에 사용되고, MQTT는 실시간 상태 보고와 제어에 사용됩니다. QMS 요청은 로컬 서버가 수신한 뒤 빈 `HTTP 200` 응답을 반환합니다. `1.0.68` 패치는 세 서비스의 목적지와 포트, 펌웨어 표시 버전을 변경하고 확장 상태값을 추가하며 원본 파일의 체크섬도 다시 계산합니다.

### 생성 및 설치

1. 카드에 표시된 Local Server IP가 Docker 서버의 고정 IPv4 주소인지 확인합니다. 필요한 경우에만 **Override**를 선택해 직접 입력합니다.
2. **Enable & Build**를 누릅니다. 서버는 기존 Root CA를 유지하면서 현재 IP가 SAN에 포함된 MEF·MQTT·QMS 인증서를 다시 발급하고, 같은 IP를 사용하는 `comMTTL-W01_1.0.68.fwr`를 생성합니다.
...
- **Enable & Build**: 패치 상태를 저장하고, 현재 IP용 인증서와 `1.0.68`을 새로 생성하며, 로컬 MEF가 해당 펌웨어를 OTA 대상으로 응답하게 합니다.
...
- 생성된 `1.0.68`은 `/srv/mttl/data/firmware-patch/`에 저장됩니다. 컨테이너 이미지를 교체해도 `/srv/mttl/data`를 유지하면 함께 보존됩니다.

### 순정 펌웨어로 복원

DNAT를 통해 로컬 서버에 연결되는 기기는 패치를 **Disable**한 뒤 OTA 확인이 발생하면 `1.0.66`으로 복원됩니다. Direct-local `1.0.68` 기기는 설정된 서버 IP에 직접 연결하므로, 서버가 같은 IP에서 실행 중이면 **Disable** 후 OTA 확인을 통해 복원할 수 있습니다. 서버에 도달하지 못하는 경우에는 OTA Tool 또는 임시 DNAT 구성이 필요합니다.
...
- 이미 설치된 `1.0.68`은 이전 IP를 계속 사용하므로 인증서만 갱신해서는 새 서버를 찾을 수 없습니다.
- 이전 IP에서 서버를 잠시 운영할 수 있다면 OTA로 새 `1.0.68`을 제공할 수 있습니다.
...
- 전체/채널별 현재 소비전력 확인
- 누적 전력량 Meter 확인
- Direct-local `1.0.68` 적용 시 전체 전압·전체 전류와 채널별 누적 전력량·전류·온도 확인
- 4개 채널을 2열×2행 카드로 표시하고 각 채널 센서값을 세로로 배치
- 펌웨어 버전과 온라인 상태 확인
- **HA Link** 활성화/비활성화
- 기기 카드 삭제

연결이 끊기면 기본적으로 약 45초 뒤 오프라인으로 판단합니다. 오프라인 기기의 센서값은 단위를 유지한 `-`로 표시합니다. 기기 상태 변경은 SSE를 통해 대시보드에 실시간으로 반영되며, 누락에 대비해 약 30초 간격으로 다시 확인합니다. `1.0.66`에서는 확장 상태값이 제공되지 않으므로 `1.0.68` 전용 센서 항목이 표시되지 않거나 값이 없는 상태로 보일 수 있습니다.
...
펌웨어 패치를 활성화하면 OTA 대상이 Direct-local `1.0.68`로 변경됩니다. DNAT로 로컬 서버에 연결된 `1.0.66` 기기는 전체 전원을 완전히 차단했다가 다시 공급하면 OTA 확인 후 `1.0.68`로 자동 업데이트됩니다. 이미 목표 버전과 동일한 기기에는 업데이트를 제안하지 않습니다.
...
- Direct-local `1.0.68` 펌웨어의 확장 상태값(전체 전압·전류, 채널별 누적량·전류·온도) 지원
- Home Assistant MQTT Discovery에 확장 센서 추가
- 패치 활성화 시 `1.0.68`, 비활성화 시 순정 `1.0.66` OTA 응답 지원
- 대시보드에 확장 센서 표시, 오프라인 센서 표시 및 2×2 채널 카드 레이아웃 적용
- `1.0.68` 전체 센서 지원을 위한 HA `mttl-w01-1.0.68-card.js` Lovelace 카드 추가
